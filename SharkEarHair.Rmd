...
fit_lag <- brm(
  PAL_int_scale ~ lag_density_scale + sac_density_scale + utr_density_scale + s(frequency, k=5) + (1 | species/subject),
  data = earHair,
  family = gaussian(),
  prior = c(
...
#fixed effects
posterior_summary(fit_lag)

pp_check(fit_lag)

#posterior prediction
pp <- posterior_predict(fit_lag)
plot(pp)
...
tmp <- conditional_smooths(fit_lag)
plot(tmp)
bayesplot::pp_check(fit_lag)
```
